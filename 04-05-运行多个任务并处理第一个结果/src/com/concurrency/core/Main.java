...
public class Main {
    public static void main(String[] args) {
        //创建两个String对象，一个名为name，另一个名为password，使用"test"值初始化他们
        String username = "test";
        String password = "test";

        //创建两个UserValidator对象，一个名为ldapValidato，另一个名为dbValidator
        UserValidator ldapValidato = new UserValidator("LDAP");
        UserValidator dbValidator = new UserValidator("DataBase");

        //创建两个TaskValidator对象，分别为ldapTask和dbTask。分别使用ldapValidator和dbValidator初始化它们
        TaskValidator ldapTask = new TaskValidator(ldapValidato,username,password);
        TaskValidator dbTask = new TaskValidator(dbValidator,username,password);

        //创建TaskValidator队列，添加两个已经创建的对象(ldapTask和dbTask)
        ...
        //使用shutdown()方法结束执行者，写入一条信息到控制台，表明程序已经结束
        executor.shutdown();
